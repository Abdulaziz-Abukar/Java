...
Create a simple console-based Library Management System that allows users to perform the following operations:


Add a Book: Add a new book to the library.
View Books: Display all books in the library.
Borrow a Book: Borrow a book from the library (mark as borrowed).
Return a Book: Return a book to the library (mark as available).
Save to File: Save the library's book list to a file.
Load from File: Load the library's book list from a file.


Requirements
Use classes and inheritance to model books and users.
Implement exception handling to manage invalid inputs and file I/O errors.
Utilize file I/O to save and load the library's data.
Apply polymorphism to handle different types of users (e.g., students, teachers).


Step-by-Step Guide
1. Define Classes and Interfaces
Book Class: Represents a book with properties like title, author, isbn, and isBorrowed.
User Interface: Define an interface LibraryUser with methods for borrowBook() and returnBook().
Student and Teacher Classes: Implement the LibraryUser interface to represent different user types.
2. Implement Exception Handling
InvalidInputException: Custom exception to handle invalid user inputs.
FileHandlingException: Custom exception for file-related errors.
3. Implement File I/O
Use file I/O to save the list of books to a file and load them back into the system.
4. Main Program Logic
Provide a menu for users to choose from different operations.
Use loops and control flow to handle user inputs and actions.
 */
